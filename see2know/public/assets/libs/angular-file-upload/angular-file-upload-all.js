		...
		return function() {
			...
			if (arguments[0] && arguments[0].__isFileAPIShim) {
				...
				var config = {
					...
					complete: function(error, fileApiXHR) {
						xhr.__completed = true;
						if (!error && xhr.__listeners['load']) 
							xhr.__listeners['load']({type: 'load', loaded: xhr.__loaded, total: xhr.__total, target: xhr, lengthComputable: true});
						if (!error && xhr.__listeners['loadend']) 
							xhr.__listeners['loadend']({type: 'loadend', loaded: xhr.__loaded, total: xhr.__total, target: xhr, lengthComputable: true});
						if (error === 'abort' && xhr.__listeners['abort']) 
							xhr.__listeners['abort']({type: 'abort', loaded: xhr.__loaded, total: xhr.__total, target: xhr, lengthComputable: true});
						if (fileApiXHR.status !== undefined) redefineProp(xhr, 'status', function() {return (fileApiXHR.status == 0 && error && error !== 'abort') ? 500 : fileApiXHR.status});
						...
						var resp = fileApiXHR.responseText || (error && fileApiXHR.status == 0 && error !== 'abort' ? error : undefined);
						redefineProp(xhr, 'responseText', function() {return resp});
						redefineProp(xhr, 'response', function() {return resp});
						if (error) redefineProp(xhr, 'error', function() {return error});
